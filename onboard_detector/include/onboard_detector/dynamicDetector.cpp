...
namespace onboardDetector{
    dynamicDetector::dynamicDetector(){
        this->_node = rclcpp::Node::make_shared("dynamic_detector_node");
    ...
    dynamicDetector::dynamicDetector(const std::shared_ptr<rclcpp::Node>& node){
        this->_node = node;
        this->ns_ = "onboard_detector";
        this->hint_ = "[onboardDetector]";
        this->initParam();
    ...
    std::shared_ptr<rclcpp::Node> dynamicDetector::getNode() const{
        return this->_node;
    }
    ...
    void dynamicDetector::initParam(){
        ...
        if (!allZeroLidar) {
            ...
            // Fallback to depth camera transform if lidar transform not present
            this->body2Lidar_ = this->body2CamDepth_;
        }
    ...
    std::cout << this->hint_ << ": The retrack similarity threshold is set to: " << this->simThreshRetrack_ << std::endl;    
        ...
        if (kfParams.size() == 7) {
            ...
            this->eQVel_ = kfParams[2];
            this->eQAcc_ = kfParams[3];
            this->eRPos_ = kfParams[4];
        ...
        defaultW << 2, 2, 2, 1, 1, 1, 0.5, 0.5, 0.5, 0.5;
        this->featureWeights_ = defaultW;
        if (featW.size() == 10) {
            for (int i = 0; i < 10; ++i) this->featureWeights_(i) = featW[i];
        } else if (featW.size() == 9) {
            // Map 3pos + 3size + 3pc(centroid) to our 10-dim; keep cluster-length weight (idx 6) default
            for (int i = 0; i < 6; ++i) this->featureWeights_(i) = featW[i];
            // Keep index 6 (pc length) as default
            this->featureWeights_(7) = featW[6];
            this->featureWeights_(8) = featW[7];
            this->featureWeights_(9) = featW[8];
        }
        std::cout << this->hint_ << ": Feature weights loaded." << std::endl;
    }
    ...
    void dynamicDetector::registerCallback(){
        ...
        this->serviceCbGroup_ = this->_node->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
            ...
            if (this->localizationMode_ == 0){
                this->poseSub_ = std::make_shared<message_filters::Subscriber<geometry_msgs::msg::PoseStamped>>(this->_node, this->poseTopicName_,rmw_qos_profile_sensor_data);
                ...
                this->lidarPoseSync_ = std::make_shared<message_filters::Synchronizer<lidarPoseSync>>(lidarPoseSync(100), *this->lidarSub_, *this->poseSub_);
                this->lidarPoseSync_->registerCallback(std::bind(&dynamicDetector::lidarPoseCB, this, std::placeholders::_1, std::placeholders::_2));
            }
            else{
                this->odomSub_ = std::make_shared<message_filters::Subscriber<nav_msgs::msg::Odometry>>(this->_node, this->odomTopicName_, rmw_qos_profile_sensor_data);
                ...
                this->lidarOdomSync_ = std::make_shared<message_filters::Synchronizer<lidarOdomSync>>(lidarOdomSync(100), *this->lidarSub_, *this->odomSub_);
    ...
    (void)0; // placeholder: no latestCloud_ in ROS2 version
        ...
        // transform to map using sensor pose
        Eigen::Matrix4d lidarPoseMatrix;
    this->getSensorPose(pose, this->body2Lidar_, lidarPoseMatrix);
    Eigen::Affine3d transform = Eigen::Affine3d::Identity();
    transform.translation() = Eigen::Vector3d(lidarPoseMatrix(0,3), lidarPoseMatrix(1,3), lidarPoseMatrix(2,3));
    transform.linear() = lidarPoseMatrix.block<3,3>(0,0);
        ...
        // voxel downsample until below threshold
        ...
        while (int(downsampledCloud->size()) > this->downSampleThresh_) {
            float leaf = sor.getLeafSize().x() * 1.1f;
    ...
    void dynamicDetector::lidarOdomCB(const sensor_msgs::msg::PointCloud2::ConstSharedPtr& cloudMsg, const nav_msgs::msg::Odometry::ConstSharedPtr& odom){
        // reuse lidarPoseCB logic but get pose from odom
        // create a temporary PoseStamped from odom
        auto pose = std::make_shared<geometry_msgs::msg::PoseStamped>();
        pose->header = odom->header;
        pose->pose = odom->pose.pose;
        this->lidarPoseCB(cloudMsg, pose);
    }
    ...
    void dynamicDetector::lidarDetect(){
        if (this->lidarDetector_ == NULL){
            this->lidarDetector_.reset(new onboardDetector::lidarDetector());
        ...
        if (this->lidarCloud_ != NULL){
            ...
            if (!this->lidarClusters_.empty()){
                ...
                pcl::toROSMsg(merged, outMsg);
                outMsg.header.frame_id = this->frameId_;
                this->lidarClustersPub_->publish(outMsg);
            }
        }
    }
    ...
    void dynamicDetector::filterBBoxes(){
        ...
        // Fuse remaining lidar boxes: add lidar bboxes that do not significantly overlap with existing filtered boxes
        for (size_t i = 0; i < this->lidarBBoxes_.size(); ++i) {
            ...
            for (size_t j = 0; j < filteredBBoxesTemp.size(); ++j) {
                double bestIOU;
                (void)this->getBestOverlapBBox(lidarBBox, std::vector<onboardDetector::box3D>{filteredBBoxesTemp[j]}, bestIOU);
                if (bestIOU > this->boxIOUThresh_) { overlaps = true; break; }
            }
            if (!overlaps) {
                filteredBBoxesTemp.push_back(lidarBBox);
                // lidar clusters converted into Eigen vectors for filteredPcClusters_
                if (i < this->lidarClusters_.size()){
                    std::vector<Eigen::Vector3d> pc;
                    for (auto &pt : this->lidarClusters_[i].points->points){
                        pc.emplace_back(pt.x, pt.y, pt.z);
                    }
                    filteredPcClustersTemp.push_back(pc);
                    Eigen::Vector3d center(this->lidarClusters_[i].centroid[0], this->lidarClusters_[i].centroid[1], this->lidarClusters_[i].centroid[2]);
                    filteredPcClusterCentersTemp.push_back(center);
                    Eigen::Vector3d stdv(this->lidarClusters_[i].eigen_values[0], this->lidarClusters_[i].eigen_values[1], this->lidarClusters_[i].eigen_values[2]);
                    filteredPcClusterStdsTemp.push_back(stdv);
                }
            }
        }
    ...
    void dynamicDetector::publish3dBox(const std::vector<box3D>& boxes, const rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr& publisher, double r, double g, double b) {
        ...
        line.header.frame_id = this->frameId_;
    ...
    void dynamicDetector::publishHistoryTraj(){
        ...
        for (size_t i=0; i<this->boxHist_.size(); ++i){
            ...
            traj.header.frame_id = this->frameId_;
    ...
    void dynamicDetector::publishVelVis(){ // publish velocities for all tracked objects
        ...
        for (size_t i=0; i<this->trackedBBoxes_.size(); ++i){
            ...
            velMarker.header.frame_id = this->frameId_;
