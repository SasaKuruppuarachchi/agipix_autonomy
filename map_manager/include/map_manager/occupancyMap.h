...
namespace mapManager{
    ...
    protected:
      ...
      int sensorInputMode_; // 0: depth camera, 1: pointcloud (lidar)
      std::string ns_;
      std::string hint_;
  ...
  // mutexes for protecting sensor data when callbacks and timers run on different threads
  ...
  std::mutex pointcloudMutex_;
    ...
    public:
        ...
        occMap(const std::shared_ptr<rclcpp::Node>& node);
        void initMap();
        void initParam();
