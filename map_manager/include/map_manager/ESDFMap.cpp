...
namespace mapManager{
    ESDFMap::ESDFMap(const std::shared_ptr<rclcpp::Node>& node)
	:occMap(node)
	{
        this->ns_ = "esdf_map";
        this->hint_ = "[ESDFMap]";
	...
	void ESDFMap::registerESDFPub(){
		this->esdfPub_ = _node->create_publisher<sensor_msgs::msg::PointCloud2>(this->ns_ + "/esdf", 10);
	}

	void ESDFMap::registerESDFCallback(){
		this->esdfTimer_ = _node->create_wall_timer(50ms, std::bind(&ESDFMap::updateESDFCB, this));
		this->esdfPubTimer_ = _node->create_wall_timer(50ms, std::bind(&ESDFMap::ESDFPubCB, this));
	}
