...
class Navigation(Node):
    def __init__(self, cfg):
        ...
        self.action_pub = self.create_publisher(Twist, cmd_topic, cmd_qos)
        ...
        self.get_logger().info(f"[navRunner]: Map frame ID: {self.map_frame_id}.")
    ...
    def init_model(self, ckpt_file):
        ...
        observation_spec = Composite({
            "agents": Composite({
                "observation": Composite({
                    "state": UnboundedContinuous((observation_dim,), device=self.cfg.device), 
                    "lidar": UnboundedContinuous((1, self.lidar_hbeams, self.cfg.sensor.lidar_vbeams), device=self.cfg.device),
                    "direction": UnboundedContinuous((1, 3), device=self.cfg.device),
                    "dynamic_obstacle": UnboundedContinuous((1, self.cfg.algo.feature_extractor.dyn_obs_num, num_dim_each_dyn_obs_state), device=self.cfg.device),
                }),
            }).expand(1)
        ...
        action_spec = Composite({
            "agents": Composite({
                "action": UnboundedContinuous((action_dim,), device=self.cfg.device), 
            })
    ...
    def goal_vis_callback(self):
        ...
        msg = MarkerArray()
        goal_point = Marker()
        goal_point.header.frame_id = self.map_frame_id
