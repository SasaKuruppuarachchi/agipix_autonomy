...
namespace AutoFlight{
	...
	private:
		...
		rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr targetVisPub_; // visualization
		rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr pathPub_; // visualization
		rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr avoidancePathVisPub_; // visualization
		rclcpp::TimerBase::SharedPtr targetVisTimer_;
		rclcpp::TimerBase::SharedPtr pathVisTimer_;
		rclcpp::TimerBase::SharedPtr avoidancePathVisTimer_;
		...
		// visualization
		std::vector<visualization_msgs::msg::Marker> targetVisVec_;
		visualization_msgs::msg::MarkerArray targetVisMsg_; 
		nav_msgs::msg::Path inspectionPath_;
	...
	public:
		...
		void mapCB(const octomap_msgs::msg::Octomap &msg);

		// Visualziation
		visualization_msgs::msg::Marker getLineMarker(double x1, double y1, double z1, double x2, double y2, double z2, int id, bool hasReachTarget);
		void updateTargetVis(const std::vector<double>& range, bool hasReachTarget);
		void updatePathVis(const nav_msgs::msg::Path& path);
		void updateAvoidancePathVis(const std::vector<nav_msgs::msg::Path>& pathVec, int bestIdx);
		void publishTargetVis();
		void publishPathVis();
		void publishAvoidancePathVis();

		// helper functions
		geometry_msgs::msg::PoseStamped getForwardGoal(bool& success);
		geometry_msgs::msg::PoseStamped getForwardGoalFromPose(const geometry_msgs::msg::PoseStamped& psTarget, bool& success);
		nav_msgs::msg::Path getForwardPathFromPose(const geometry_msgs::msg::PoseStamped& psTarget, bool& success);
		nav_msgs::msg::Path getForwardPath(bool& success);
		...
		geometry_msgs::msg::PoseStamped getCurrPose();
		...
		geometry_msgs::msg::PoseStamped pointToPose(const octomap::point3d& p); // this will keep current orientation. Be careful
		void moveToPos(const geometry_msgs::msg::Point& position); // only translation movement
		void moveToAngle(const geometry_msgs::msg::Quaternion& quat);
		nav_msgs::msg::Path checkSurroundingsLeft();
		nav_msgs::msg::Path checkSurroundingsRight();
		bool executeWaypointPath(const nav_msgs::msg::Path& path, bool useYaw=false, bool onlineCollisionCheck=true);
		bool executeWaypointPathHeading(const nav_msgs::msg::Path& path, bool onlineCollisionCheck=true);
		bool executeWaypointPathToTime(const nav_msgs::msg::Path& path, double time, geometry_msgs::msg::PoseStamped& psTargetCurr, bool onlineCollisionCheck);
		bool executeAvoidancePath(const nav_msgs::msg::Path& path, bool onlineCollisionCheck=false);
		double poseDistance(const geometry_msgs::msg::PoseStamped& ps1, const geometry_msgs::msg::PoseStamped& ps2);
		double findPathLength(const nav_msgs::msg::Path& path);
		...
		void rrtPathRegenInteractive(const std::vector<double>& goalVec, nav_msgs::msg::Path& path);
		void rrtPathRegen(const std::vector<double>& goalVec, nav_msgs::msg::Path& path);
		double evaluatePointObstacleDist(const octomap::point3d& p);
		double evaluatePathMinObstacleDist(const nav_msgs::msg::Path& path);
