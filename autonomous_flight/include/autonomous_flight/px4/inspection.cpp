...
namespace AutoFlight{
	inspector::inspector(const rclcpp::Node::SharedPtr& node) : flightBase(node){
		...
		this->targetVisTimer_ = this->node_->create_wall_timer(
			std::chrono::seconds(1), std::bind(&inspector::publishTargetVis, this));

		this->pathPub_ = this->node_->create_publisher<nav_msgs::msg::Path>("/inspector/path", 100);
		this->pathVisTimer_ = this->node_->create_wall_timer(
			std::chrono::milliseconds(100), std::bind(&inspector::publishPathVis, this));

		this->avoidancePathVisPub_ = this->node_->create_publisher<visualization_msgs::msg::MarkerArray>("/inspector/avoidance_path", 100);
		this->avoidancePathVisTimer_ = this->node_->create_wall_timer(
			std::chrono::milliseconds(500), std::bind(&inspector::publishAvoidancePathVis, this));
	}

	void inspector::loadParam(){
		// Collision box
		this->node_->declare_parameter<std::vector<double>>("collision_box", std::vector<double> {0.75, 0.75, 0.3});
		this->node_->get_parameter("collision_box", this->collisionBox_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Collision Box: [%.2f, %.2f, %.2f].", this->collisionBox_[0], this->collisionBox_[1], this->collisionBox_[2]);

		// Front Safe distance
		this->node_->declare_parameter<double>("front_safe_distance", 2.0);
		this->node_->get_parameter("front_safe_distance", this->frontSafeDist_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Front safe distance to obstacle is set to: %.2fm.", this->frontSafeDist_);

		// Avoidance Safe distance
		this->node_->declare_parameter<double>("avoidance_front_safe_distance", 1.0);
		this->node_->get_parameter("avoidance_front_safe_distance", this->avoidSafeDist_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Avoidance front safe distance is set to: %.2fm.", this->avoidSafeDist_);

		// Side Safe distance
		this->node_->declare_parameter<double>("side_safe_distance", 1.5);
		this->node_->get_parameter("side_safe_distance", this->sideSafeDist_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Side safe distance to wall is set to: %.2fm.", this->sideSafeDist_);

		// Zig Zag Safe distance
		this->node_->declare_parameter<double>("zig_zag_safe_distance", 1.5);
		this->node_->get_parameter("zig_zag_safe_distance", this->zigZagSafeDist_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Zig zag safe distance to wall is set to: %.2fm.", this->zigZagSafeDist_);

		// Avoidance Online Check
		this->node_->declare_parameter<bool>("avoidance_online_collision_check", false);
		this->node_->get_parameter("avoidance_online_collision_check", this->avoidanceOnlineCheck_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: NBV avoidance online check is set to: %s.", this->avoidanceOnlineCheck_ ? "true" : "false");

		// min target area
		this->node_->declare_parameter<double>("min_target_area", 10.0);
		this->node_->get_parameter("min_target_area", this->minTargetArea_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Min target area for inspection: %.2f m^2.", this->minTargetArea_);

		// max height
		this->node_->declare_parameter<double>("max_inspection_target_height", 3.0);
		this->node_->get_parameter("max_inspection_target_height", this->maxTargetHgt_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Max height for inspection: %.2fm.", this->maxTargetHgt_);

		// max width
		this->node_->declare_parameter<double>("max_inspection_target_width", 5.0);
		this->node_->get_parameter("max_inspection_target_width", this->maxTargetWidth_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Max width for inspection: %.2fm.", this->maxTargetWidth_);

		// descend height
		this->node_->declare_parameter<double>("inspection_descend_height", 3.0);
		this->node_->get_parameter("inspection_descend_height", this->descendHgt_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Descend height for inspection: %.2fm.", this->descendHgt_);

		// desired velocity
		this->node_->declare_parameter<double>("desired_velocity", 0.3);
		this->node_->get_parameter("desired_velocity", this->desiredVel_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Desired velocity: %.2f m/s.", this->desiredVel_);

		// desired angular velocity
		this->node_->declare_parameter<double>("desired_angular_velocity", 0.3);
		this->node_->get_parameter("desired_angular_velocity", this->desiredAngularVel_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Desired angular velocity: %.2f rad/s.", this->desiredAngularVel_);

		// NBV sample number
		this->node_->declare_parameter<int>("nbv_sample_num", 10);
		this->node_->get_parameter("nbv_sample_num", this->nbvSampleNum_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: NBV sample number: %d.", this->nbvSampleNum_);

		// Sensor Range
		this->node_->declare_parameter<double>("sensor_range", 5.0);
		this->node_->get_parameter("sensor_range", this->sensorRange_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Sensor range: %.2fm.", this->sensorRange_);

		// Sensor vertical angle (degrees)
		this->node_->declare_parameter<double>("sensor_vertical_angle", 45.0);
		this->node_->get_parameter("sensor_vertical_angle", this->sensorVerticalAngle_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Sensor vertical angle: %.2f degree.", this->sensorVerticalAngle_);
		this->sensorVerticalAngle_ *= PI_const/180.0;

		// minimum forward distance
		this->node_->declare_parameter<double>("forward_min_distance", 0.5);
		this->node_->get_parameter("forward_min_distance", this->forwardMinDist_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Minimum Forward Distance: %.2fm.", this->forwardMinDist_);

		// step ascend distance
		this->node_->declare_parameter<double>("step_ascend_delta", 3.0);
		this->node_->get_parameter("step_ascend_delta", this->stepAscendDelta_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Step Ascend Delta: %.2fm.", this->stepAscendDelta_);

		// look around angle (degrees)
		this->node_->declare_parameter<double>("look_around_angle", 90.0);
		this->node_->get_parameter("look_around_angle", this->lookAroundAngle_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Look around angle: %.2f degree.", this->lookAroundAngle_);
		this->lookAroundAngle_ *= PI_const/180.0;

		// check target look around angle (degrees)
		this->node_->declare_parameter<double>("check_target_look_around_angle", 90.0);
		this->node_->get_parameter("check_target_look_around_angle", this->checkTargetLookAroundAngle_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Check target look around angle: %.2f degree.", this->checkTargetLookAroundAngle_);
		this->checkTargetLookAroundAngle_ *= PI_const/180.0;

		// start free range
		this->node_->declare_parameter<std::vector<double>>("start_free_range", std::vector<double> {1.0, 1.0, 1.0});
		this->node_->get_parameter("start_free_range", this->startFreeRange_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Start free range: [%.2f, %.2f, %.2f].", this->startFreeRange_[0], this->startFreeRange_[1], this->startFreeRange_[2]);

		// NBV sample timeout
		this->node_->declare_parameter<double>("nbv_sample_time_out", 1.0);
		this->node_->get_parameter("nbv_sample_time_out", this->sampleTimeout_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: NBV sample timeout: %.2f s.", this->sampleTimeout_);

		// NBV safety reduce factor
		this->node_->declare_parameter<double>("safe_reduce_factor", 0.5);
		this->node_->get_parameter("safe_reduce_factor", this->reduceFactor_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Safe reduce factor: %.2f.", this->reduceFactor_);

		// path regeneration
		this->node_->declare_parameter<bool>("path_regeneration", true);
		this->node_->get_parameter("path_regeneration", this->pathRegenOption_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Path regeneration is set to: %s.", this->pathRegenOption_ ? "true" : "false");

		// interactive path regeneration option
		this->node_->declare_parameter<bool>("interactive_regeneration", true);
		this->node_->get_parameter("interactive_regeneration", this->interactivePathRegen_);
		RCLCPP_INFO(this->node_->get_logger(), "[AutoFlight]: Interactive regeneration is set to: %s.", this->interactivePathRegen_ ? "true" : "false");
	...
	void inspector::initPlanner(){
		this->pwlPlanner_ = new trajPlanner::pwlTraj(this->node_);
		this->rrtPlanner_ = new globalPlanner::rrtOctomap<3>(this->node_);
	}
	...
	void inspector::mapCB(const octomap_msgs::msg::Octomap &msg){
	...
	void inspector::updateAvoidancePathVis(const std::vector<nav_msgs::Path>& pathVec, int bestIdx){
		...
		for (nav_msgs::Path path : pathVec){
			for (size_t i=0; i<path.poses.size()-1; ++i){
				...
				lineMarker.type = visualization_msgs::Marker::LINE_LIST;
				lineMarker.action = visualization_msgs::Marker::ADD;
				lineMarker.lifetime = rclcpp::Duration::from_seconds(1.0);
				lineMarker.points = lineVec;
