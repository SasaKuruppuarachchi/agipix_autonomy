...
namespace controller{
	trackingController::trackingController(const rclcpp::Node::SharedPtr& node) : node_(node){
		this->initParam();
		this->registerPub();
	...
	void trackingController::initParam(){
		// body rate control
		this->bodyRateControl_ = this->node_->declare_parameter<bool>("controller.body_rate_control", true);
		cout << "[trackingController]: Body rate control is set to: " << this->bodyRateControl_  << endl;

		// attitude control
		this->attitudeControl_ = this->node_->declare_parameter<bool>("controller.attitude_control", true);
		...
		// P for Position
		std::vector<double> pPosTemp = this->node_->declare_parameter<std::vector<double>>("controller.position_p", {1.0, 1.0, 1.0});
		if (pPosTemp.size() >= 3){
			...
			this->pPos_ = Eigen::Vector3d(1.0, 1.0, 1.0);
		}
		cout << "[trackingController]: Position p is set to: " << "[" << this->pPos_(0) << ", " << this->pPos_(1) << ", " << this->pPos_(2) << "]." << endl;

		// I for Position
		std::vector<double> iPosTemp = this->node_->declare_parameter<std::vector<double>>("controller.position_i", {0.0, 0.0, 0.0});
		if (iPosTemp.size() >= 3){
			...
			this->iPos_ = Eigen::Vector3d(0.0, 0.0, 0.0);
		}
		cout << "[trackingController]: Position i is set to: " << "[" << this->iPos_(0) << ", " << this->iPos_(1) << ", " << this->iPos_(2) << "]." << endl;

		// D for Position
		std::vector<double> dPosTemp = this->node_->declare_parameter<std::vector<double>>("controller.position_d", {0.0, 0.0, 0.0});
		if (dPosTemp.size() >= 3){
			...
			this->dPos_ = Eigen::Vector3d(0.0, 0.0, 0.0);
		}
		cout << "[trackingController]: Position d is set to: " << "[" << this->dPos_(0) << ", " << this->dPos_(1) << ", " << this->dPos_(2) << "]." << endl;


		// P for Velocity
		std::vector<double> pVelTemp = this->node_->declare_parameter<std::vector<double>>("controller.velocity_p", {1.0, 1.0, 1.0});
		if (pVelTemp.size() >= 3){
			...
			this->pVel_ = Eigen::Vector3d(1.0, 1.0, 1.0);
		}
		cout << "[trackingController]: Velocity p is set to:" << "[" << this->pVel_(0) << ", " << this->pVel_(1) << ", " << this->pVel_(2) << "]." << endl; 

		// I for Velocity
		std::vector<double> iVelTemp = this->node_->declare_parameter<std::vector<double>>("controller.velocity_i", {0.0, 0.0, 0.0});
		if (iVelTemp.size() >= 3){
			...
			this->iVel_ = Eigen::Vector3d(0.0, 0.0, 0.0);
		}
		cout << "[trackingController]: Velocity i is set to:" << "[" << this->iVel_(0) << ", " << this->iVel_(1) << ", " << this->iVel_(2) << "]." << endl; 

		// D for Velocity
		std::vector<double> dVelTemp = this->node_->declare_parameter<std::vector<double>>("controller.velocity_d", {0.0, 0.0, 0.0});
		if (dVelTemp.size() >= 3){
			...
			this->dVel_ = Eigen::Vector3d(0.0, 0.0, 0.0);
		}
		...
		this->attitudeControlTau_ = this->node_->declare_parameter<double>("controller.attitude_control_tau", 0.3);
		cout << "[trackingController]: Attitude control tau is set to: " << this->attitudeControlTau_  << endl;

		// Estimated Maximum acceleration
		this->hoverThrust_ = this->node_->declare_parameter<double>("controller.hover_thrust", 0.3);
		cout << "[trackingController]: Hover thrust is set to: " << this->hoverThrust_  << endl;

		// Display message
		this->verbose_ = this->node_->declare_parameter<bool>("controller.verbose", false);
	...
	void trackingController::registerPub(){
		...
		this->histTrajVisPub_ = this->node_->create_publisher<nav_msgs::msg::Path>("/tracking_controller/trajectory_history", 1);

		// target pose visualization publisher
		this->targetVisPub_ = this->node_->create_publisher<geometry_msgs::msg::PoseStamped>("/tracking_controller/target_pose", 1);
	
		// target trajectory history publisher
		this->targetHistTrajVisPub_ = this->node_->create_publisher<nav_msgs::msg::Path>("/tracking_controller/target_trajectory_history", 1); 
	...
	void trackingController::registerCallback(){
		...
		this->imuCbGroup_ = this->node_->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
		this->targetCbGroup_ = this->node_->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
		this->cmdCbGroup_ = this->node_->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
		this->thrustCbGroup_ = this->node_->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
		...
		rclcpp::SubscriptionOptions targetOpts;
		targetOpts.callback_group = this->targetCbGroup_;
		this->targetSub_ = this->node_->create_subscription<tracking_controller::msg::Target>(
			"/autonomous_flight/target_state",
			rclcpp::QoS(1),
			std::bind(&trackingController::targetCB, this, std::placeholders::_1),
			targetOpts);
	
		// controller publisher timer
		this->cmdTimer_ = this->node_->create_wall_timer(
			10ms,
			std::bind(&trackingController::cmdCB, this),
			this->cmdCbGroup_);

		if (not this->accControl_){
			// auto thrust esimator timer
			this->thrustEstimatorTimer_ = this->node_->create_wall_timer(
				10ms,
				std::bind(&trackingController::thrustEstimateCB, this),
				this->thrustCbGroup_);
		}
		// visualization timer
		this->visTimer_ = this->node_->create_wall_timer(
			33ms,
			std::bind(&trackingController::visCB, this),
			this->visCbGroup_);
	}
	...
	void trackingController::targetCB(const tracking_controller::msg::Target::SharedPtr target){
		this->target_ = *target;
	...
	void trackingController::thrustEstimateCB(){
		...
		if (this->kfFirstTime_){
			...
			this->kfStartTime_ = this->node_->now();
			return;
		}
		...
		if (std::abs(prevMinThrust - prevMaxThrust) < 0.005){
			if (newHoverThrust > 0 and newHoverThrust < 1.0){
				...
				double estimatedTime = (currTime - this->kfStartTime_).seconds();
				if (this->verbose_){
	...
	void trackingController::computeAttitudeAndAccRef(Eigen::Vector4d& attitudeRefQuat, Eigen::Vector3d& accRef){
		...
		*/
		...
		if (this->target_.type_mask == tracking_controller::msg::Target::IGNORE_ACC_VEL){
			velocityError *= 0.0;
		...
		if (this->target_.type_mask == tracking_controller::msg::Target::IGNORE_ACC_VEL ||
			this->target_.type_mask == tracking_controller::msg::Target::IGNORE_ACC){
			accTarget *= 0.0;
		}
	...
	void trackingController::publishVelAndAccVis(){
		...
		if (this->velFirstTime_){
			this->velPrevTime_ = this->node_->now();
		...
		velAndAccVisMsg.lifetime = rclcpp::Duration::from_seconds(0.05);
