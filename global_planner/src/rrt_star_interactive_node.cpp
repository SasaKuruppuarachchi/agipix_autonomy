...
class RrtStarInteractiveNode {
public:
	RrtStarInteractiveNode()
	: node_(rclcpp::Node::make_shared("rrt_star_test_node"))
	, rrtStarPlanner_(node_) {
		...
		goalPointSub_ = node_->create_subscription<geometry_msgs::msg::PoseStamped>(
			...
			std::bind(&RrtStarInteractiveNode::goalPointCB, this, std::placeholders::_1), options);
		startPointSub_ = node_->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
			"/initialpose", 1000,
			std::bind(&RrtStarInteractiveNode::startPointCB, this, std::placeholders::_1), options);

		posePub_ = node_->create_publisher<geometry_msgs::msg::PoseStamped>("/trajectory_pose", 1000);
		...
		startVisTimer_ = node_->create_wall_timer(
			std::chrono::milliseconds(100), std::bind(&RrtStarInteractiveNode::publishStartVis, this), startVisCbGroup_);
		goalVisTimer_ = node_->create_wall_timer(
			std::chrono::milliseconds(100), std::bind(&RrtStarInteractiveNode::publishGoalVis, this), goalVisCbGroup_);
		planTimer_ = node_->create_wall_timer(
			std::chrono::milliseconds(100), std::bind(&RrtStarInteractiveNode::planTimerCB, this), planCbGroup_);
...
private:
	...
	void planTimerCB(){
		...
		rrtStarPlanner_.updateStart(startPoint);
		rrtStarPlanner_.updateGoal(goalPoint);
	...
	globalPlanner::rrtStarOctomap<kDim> rrtStarPlanner_;
