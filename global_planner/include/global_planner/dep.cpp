...
namespace globalPlanner{
	DEP::DEP(const rclcpp::Node::SharedPtr &node) : node_(node){
		this->ns_ = "DEP";
		this->hint_ = "[DEP]";
		this->initParam();
		this->initModules();
	...
	void DEP::initParam(){
		// odom topic name
		this->odomTopic_ = declareAndGetParam<std::string>(this->node_, "odom_topic", "/CERLAB/quadcopter/odom");
		cout << this->hint_ << ": Odom topic: " << this->odomTopic_ << endl;

		// local sample region min
		std::vector<double> localRegionMinTemp = declareAndGetParam<std::vector<double>>(this->node_, "local_region_min", {-5.0, -5.0, -2.0});
		...
		std::vector<double> localRegionMaxTemp = declareAndGetParam<std::vector<double>>(this->node_, "local_region_max", {5.0, 5.0, 2.0});
		...
		std::vector<double> globalRegionMinTemp = declareAndGetParam<std::vector<double>>(this->node_, "global_region_min", {-20.0, -20.0, 0.0});
		...
		std::vector<double> globalRegionMaxTemp = declareAndGetParam<std::vector<double>>(this->node_, "global_region_max", {20.0, 20.0, 3.0});
	...
	void DEP::registerCallback(){
		...
		this->visTimer_ = this->node_->create_wall_timer(
			std::chrono::milliseconds(100), std::bind(&DEP::visCB, this), this->visCbGroup_);
	...
	nav_msgs::msg::Path DEP::getBestPath(){
		nav_msgs::msg::Path bestPath;
  ...
  //   	params.minConvexity = 0.1;;
	...
	void DEP::buildRoadMap(){
		...
		while (rclcpp::ok() and countFrontierFailure < this->frontierSampleThresh_ and sampleWeights.size() != 0){
		...
		while (rclcpp::ok() and not saturate){
			if (regionSaturate){
	...
	void DEP::odomCB(const nav_msgs::msg::Odometry::SharedPtr odom){
		this->odom_ = *odom;
	...
	void DEP::publishRoadmap(){
		visualization_msgs::msg::MarkerArray roadmapMarkers;
		...
		for (std::shared_ptr<PRM::Node> n : this->prmNodeVec_){
			...
			voxelNumText.action = visualization_msgs::msg::Marker::ADD;
			...
			voxelNumText.lifetime = rclcpp::Duration::from_seconds(0.5);
			++countVoxelNumText;
		...
		for (size_t i=0; i<this->goalCandidates_.size(); ++i){
			...
			goalCandidatePoint.action = visualization_msgs::msg::Marker::ADD;
			...
			goalCandidatePoint.lifetime = rclcpp::Duration::from_seconds(0.5);
	...
	void DEP::publishCandidatePaths(){
		visualization_msgs::msg::MarkerArray candidatePathMarkers;
		int countNodeNum = 0;
		int countLineNum = 0;
	...
	void DEP::publishBestPath(){
		visualization_msgs::msg::MarkerArray bestPathMarkers;
		int countNodeNum = 0;
		int countLineNum = 0;
	...
	void DEP::publishFrontier(){
		...
		for (int i=0; i<int(this->frontierPointPairs_.size()); ++i){
			...
			range.type = visualization_msgs::msg::Marker::SPHERE;
			...
			range.lifetime = rclcpp::Duration::from_seconds(0.5);
			range.scale.x = dist;
			range.scale.y = dist;
			range.scale.z = 0.1;
			range.color.a = 0.4;
