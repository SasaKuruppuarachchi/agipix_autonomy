...
namespace globalPlanner{
	...
	public:
		...
		rrtBase(const rclcpp::Node::SharedPtr &node, KDTree::Point<N> start, KDTree::Point<N> goal, std::vector<double> collisionBox, std::vector<double> envBox, double delQ, double dR, double connectGoalRatio, double timeout);

		rrtBase(const rclcpp::Node::SharedPtr &node, std::vector<double> start, std::vector<double> goal,  std::vector<double> collisionBox, std::vector<double> envBox, double delQ, double dR, double connectGoalRatio, double timeout);
		...
		void updateStart(const geometry_msgs::msg::Pose& newStart);
		...
		void updateGoal(const geometry_msgs::msg::Pose& newGoal);
	...
	// Constructor:
	template <std::size_t N>
	rrtBase<N>::rrtBase(const rclcpp::Node::SharedPtr &node, KDTree::Point<N> start, KDTree::Point<N> goal, std::vector<double> collisionBox, std::vector<double> envBox, double delQ, double dR, double connectGoalRatio, double timeout) 
		: node_(node), collisionBox_(collisionBox), envBox_(envBox), delQ_(delQ), dR_(dR), connectGoalRatio_(connectGoalRatio), timeout_(timeout){
		this->start_ = start;
		this->goal_ = goal;
		this->emptyToken_[0] = -11311; 
		this->parent_[start_] = this->emptyToken_; // set start parent to NULL
	...
	template <std::size_t N>
	rrtBase<N>::rrtBase(const rclcpp::Node::SharedPtr &node, std::vector<double> start, std::vector<double> goal,  std::vector<double> collisionBox, std::vector<double> envBox, double delQ, double dR, double connectGoalRatio, double timeout)
		: node_(node), collisionBox_(collisionBox), envBox_(envBox), delQ_(delQ), dR_(dR), connectGoalRatio_(connectGoalRatio), timeout_(timeout){
		KDTree::Point<N> startp = KDTree::vec2Point<N>(start);
		KDTree::Point<N> goalp = KDTree::vec2Point<N>(goal);
		this->start_ = startp;
	...
	template <std::size_t N>
	void rrtBase<N>::updateStart(const geometry_msgs::msg::Pose& newStart){
	...
	template <std::size_t N>
	void rrtBase<N>::updateGoal(const geometry_msgs::msg::Pose& newGoal){
