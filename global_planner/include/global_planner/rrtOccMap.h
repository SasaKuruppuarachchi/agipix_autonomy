...
namespace globalPlanner{
	...
	public:
		rrtOccMap(const rclcpp::Node::SharedPtr &node);
		~rrtOccMap();
		void initParam();
		void registerPub();
		...
		void makePlan(nav_msgs::msg::Path& path);
		...
		bool hasNewGoal(const geometry_msgs::msg::Pose& goal);
	...
	template <std::size_t N>
	rrtOccMap<N>::rrtOccMap(const rclcpp::Node::SharedPtr &node) : rrtBase<N>(), node_(node){
		this->initParam();
		this->registerPub();
	...
	void rrtOccMap<N>::initParam(){
		...
		// Timeout:
		this->timeout_ = declareAndGetParam<double>(this->node_, "timeout", 0.5);
		...
		// goal check
		this->passGoalCheck_ = declareAndGetParam<bool>(this->node_, "pass_goal_check", false);
	...
	void rrtOccMap<N>::registerPub(){
		this->rrtVisPub_ = this->node_->create_publisher<visualization_msgs::msg::MarkerArray>("rrt/planned_path", 100);
	}
	...
	void rrtOccMap<N>::registerCallback(){
		...
		this->visTimer_ = this->node_->create_wall_timer(
			std::chrono::milliseconds(100), std::bind(&rrtOccMap<N>::visCB, this), this->visCbGroup_);
	}
	...
	template <std::size_t N>
	void rrtOccMap<N>::makePlan(nav_msgs::msg::Path& path){
	...
	template <std::size_t N>
	bool rrtOccMap<N>::hasNewGoal(const geometry_msgs::msg::Pose& goal){
	...
	template <std::size_t N>
	void rrtOccMap<N>::pathMsgConverter(const std::vector<KDTree::Point<N>>& pathTemp, nav_msgs::msg::Path& path){
	...
	template <std::size_t N>
	void rrtOccMap<N>::visCB(){
		this->publishRRTPath();
	}

	template <std::size_t N>
	void rrtOccMap<N>::publishRRTPath(){
		std::vector<visualization_msgs::msg::Marker> pathVisVec;
