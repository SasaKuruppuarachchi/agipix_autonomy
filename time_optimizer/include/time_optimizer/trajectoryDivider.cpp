...
namespace timeOptimizer{
	trajDivider::trajDivider(const rclcpp::Node::SharedPtr& node) : node_(node){
		this->registerPub();
	...
	void trajDivider::registerPub(){
		this->brakingZoneVisPub_ = this->node_->create_publisher<visualization_msgs::msg::MarkerArray>("trajDivider/braking_zone", 10);
		this->kdtreeRangeVisPub_ = this->node_->create_publisher<visualization_msgs::msg::MarkerArray>("trajDivider/kdtree_range", 10);
	}

	void trajDivider::registerCallback(){
		this->timerCallbackGroup_ = this->node_->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
		this->visTimer_ = this->node_->create_wall_timer(
			33ms,
			std::bind(&trajDivider::visCB, this),
			this->timerCallbackGroup_);
	}

	void trajDivider::visCB(){
	...
	void trajDivider::run(std::vector<std::pair<double, double>>& tInterval, std::vector<double>& obstacleDist){
		...
		cout << "[trajDivider]: Divider run time: " << (endTime - startTime).seconds() << endl;
	}
		...
		if (this->complete_){
			...
			for (size_t i=0; i<this->trajectory_.size(); ++i){
				...
				// if (this->mask_[i]){
					...
					point.lifetime = rclcpp::Duration(0, 100000000);
					point.scale.x = 0.2;
					point.scale.y = 0.2;
					point.scale.z = 0.2;
					point.color.a = 1.0;
	...
	void trajDivider::publishKDTreeRangeMsg(){
		if (this->complete_){
			...
			range.ns = "range box";
			range.id = 0;
			range.type = visualization_msgs::msg::Marker::CUBE;
			...
			// range.lifetime = rclcpp::Duration(0, 100000000);
