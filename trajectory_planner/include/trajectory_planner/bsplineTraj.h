...
namespace trajPlanner{
	...
	private:
		...
		rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr controlPointsVisPub_;
		rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr currTrajVisPub_;
		rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr astarVisPub_;
		rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr guidePointsVisPub_;
		...
		rclcpp::Logger logger_{rclcpp::get_logger("bsplineTraj")};
	...
	public:
		...
		bsplineTraj();
		bsplineTraj(const rclcpp::Node::SharedPtr& node);
		...
		bool inputPathCheck(const nav_msgs::msg::Path & path, nav_msgs::msg::Path& adjustedPath, double dt, double& finalTime);
		bool fillPath(const nav_msgs::msg::Path& path, nav_msgs::msg::Path& adjustedPath);
		bool updatePath(const nav_msgs::msg::Path& adjustedPath, const std::vector<Eigen::Vector3d>& startEndConditions);
		...
		bool makePlan();
		bool makePlan(nav_msgs::msg::Path& trajectory, bool yaw=true);
		void clear();
		...
		geometry_msgs::msg::PoseStamped getPose(double t, bool yaw=true);
		double getDuration();
		double getTimestep();
		...
		nav_msgs::msg::Path evalTrajToMsg(bool yaw=true); // evaluate current trajectory based on the control point
		nav_msgs::msg::Path evalTrajToMsg(double dt, bool yaw=true);
		void pathMsgToEigenPoints(const nav_msgs::msg::Path& path, std::vector<Eigen::Vector3d>& points);
		void eigenPointsToPathMsg(const std::vector<Eigen::Vector3d>& points, nav_msgs::msg::Path& path);
	...
	inline int bsplineTraj::findCollisionSegIndex(const std::vector<std::pair<int, int>>& collisionSeg, int idx){
		...
		for (std::pair<int, int> seg : collisionSeg){
			...
			RCLCPP_ERROR(logger_, "Index ERROR. Not exist.");
		return -1;
	}	
