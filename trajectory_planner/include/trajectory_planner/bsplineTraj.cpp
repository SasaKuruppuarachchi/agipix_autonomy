...
namespace trajPlanner{
	...
	bsplineTraj::bsplineTraj(const rclcpp::Node::SharedPtr& node) : node_(node){
	...
	void bsplineTraj::init(const rclcpp::Node::SharedPtr& node){
		this->node_ = node;
		this->clock_ = node_->get_clock();
		this->logger_ = node_->get_logger();
		this->initParam();
	...
	void bsplineTraj::initParam(){
		...
		if (not declareParam("bspline_traj/timestep", 0.1, this->ts_)){
			this->ts_ = 0.1;
		...
		if (not declareParam("bspline_traj/distance_threshold", 0.5, this->dthresh_)){
			this->dthresh_ = 0.5;
		...
		// maximum velocity
		if (not declareParam("bspline_traj/max_vel", 1.0, this->maxVel_)){
			this->maxVel_ = 1.0;
		...
		if (not declareParam("bspline_traj/max_acc", 0.5, this->maxAcc_)){
			this->maxAcc_ = 0.5;
		...
		if (not declareParam("bspline_traj/weight_distance", 0.5, this->weightDistance_)){
		...
		if (not declareParam("bspline_traj/weight_smoothness", 1.0, this->weightSmoothness_)){
		...
		if (not declareParam("bspline_traj/weight_feasibility", 1.0, this->weightFeasibility_)){
		...
		if (not declareParam("bspline_traj/weight_dynamic_obstacle", 1.0, this->weightDynamicObstacle_)){
		...
		if (not declareParam("bspline_traj/plan_in_z_axis", true, this->planInZAxis_)){
		...
		// min height
		if (not declareParam("bspline_traj/min_height", 0.5, this->minHeight_)){
			this->minHeight_ = 0.5;
		...
		// max height
		if (not declareParam("bspline_traj/max_height", 2.0, this->maxHeight_)){
			this->maxHeight_ = 2.0;
		...
		if (not declareParam("bspline_traj/uncertain_aware_factor", 2.0, this->uncertainAwareFactor_)){
		...
		// prediction horizon
		if (not declareParam("bspline_traj/prediction_horizon", 2.0, this->predHorizon_)){
		...
		if (not declareParam("bspline_traj/distance_threshold_dynamic", 1.0, this->distThreshDynamic_)){
		...
		if (not declareParam("bspline_traj/max_path_length", 7.0, this->maxPathLength_)){
		...
		if (not declareParam("bspline_traj/max_obstacle_size", std::vector<double>{10.0, 10.0, 10.0}, maxObstacleSizeTemp)){
	...
	void bsplineTraj::registerPub(){
		this->controlPointsVisPub_ = this->node_->create_publisher<visualization_msgs::msg::MarkerArray>("bspline_traj/control_points", rclcpp::QoS(10));
		this->currTrajVisPub_ = this->node_->create_publisher<nav_msgs::msg::Path>("bspline_traj/trajectory", rclcpp::QoS(10));
		this->astarVisPub_ = this->node_->create_publisher<visualization_msgs::msg::MarkerArray>("bspline_traj/astar_path", rclcpp::QoS(10));
		this->guidePointsVisPub_ = this->node_->create_publisher<visualization_msgs::msg::MarkerArray>("bspline_traj/guide_points_and_directions", rclcpp::QoS(10));
		this->inputTrajPub_ = this->node_->create_publisher<nav_msgs::msg::Path>("bspline_traj/input_trajectory", rclcpp::QoS(10));
		this->inputTrajPointPub_ = this->node_->create_publisher<visualization_msgs::msg::MarkerArray>("bspline_traj/input_trajectory_point", rclcpp::QoS(10));
	}
	...
	void bsplineTraj::registerCallback(){
		...
		this->visTimer_ = this->node_->create_wall_timer(
			...
			std::bind(&bsplineTraj::visCB, this),
	...
	void bsplineTraj::setMap(const std::shared_ptr<mapManager::occMap>& map){
		...
		this->pathSearch_->setLogger(this->logger_);
	...
	bool bsplineTraj::inputPathCheck(const nav_msgs::msg::Path & path, nav_msgs::msg::Path& adjustedPath, double dt, double& finalTime){	
	...
	bool bsplineTraj::fillPath(const nav_msgs::msg::Path& path, nav_msgs::msg::Path& adjustedPath){ // use linear interpotation to make input trajectory size = 4
		...
		if (int(path.poses.size()) == 2){
			...
			geometry_msgs::msg::PoseStamped ps1, ps2, ps3, ps4;
		...
		if (int(path.poses.size()) == 3){
			...
			geometry_msgs::msg::PoseStamped ps1, ps2, ps3, ps4, ps5;
			ps1.pose.position.x = ps(0); ps1.pose.position.y = ps(1); ps1.pose.position.z = ps(2);
			ps2.pose.position.x = pm1(0); ps2.pose.position.y = pm1(1); ps2.pose.position.z = pm1(2);
	...
	bool bsplineTraj::updatePath(const nav_msgs::msg::Path& adjustedPath, const std::vector<Eigen::Vector3d>& startEndConditions){
		if (adjustedPath.poses.empty()){
			cout << "[bsplineTraj]: Input path is empty." << endl;
			return false;
		}
		...
		// nav_msgs::msg::Path inputPath = adjustedPath;
	...
	bool bsplineTraj::makePlan(){
		...
		cout << "[BsplineTraj]: Total time: " << (endTime - startTime).seconds() << endl;
		return true;
	}
		...
		for (size_t i=0; i<collisionSeg.size(); ++i){
			...
			for (int controlPointIdx=seg.first+1; controlPointIdx<seg.second; ++controlPointIdx){ // iterate through all collision control points
				...
				if (not findGuidePoint){
					RCLCPP_ERROR(this->logger_, "[BsplineTraj]: Impossible Assignment. Something wrong!");
				}
			}
	...
	bool bsplineTraj::optimizeTrajectory(){
		...
		while (rclcpp::ok()){
			...
			rclcpp::Time currTime = this->clock_->now();
			if ((currTime - startTime).seconds() > 0.03){
	...
	void bsplineTraj::publishGuidePoints(){
		visualization_msgs::msg::MarkerArray msg;
		std::vector<visualization_msgs::msg::Marker> msgVec;
		visualization_msgs::msg::Marker point;
		...
		for (int i=0; i<this->optData_.controlPoints.cols(); ++i){
			for (size_t j=0; j<this->optData_.guidePoints[i].size(); ++j){
				...
				pointG.type = visualization_msgs::msg::Marker::SPHERE;
				pointG.action = visualization_msgs::msg::Marker::ADD;
				pointG.pose.position.x = p(0);
				pointG.pose.position.y = p(1);
				pointG.pose.position.z = p(2);
				pointG.lifetime = rclcpp::Duration::from_seconds(0.5);
				pointG.scale.x = 0.05;
				pointG.scale.y = 0.05;
				pointG.scale.z = 0.05;
				pointG.color.a = 1.0;
	...
	void bsplineTraj::publishInputTraj(){
		if (this->inputPathVis_.size() != 0){
			nav_msgs::msg::Path inputTrajPathMsg;
			this->eigenPointsToPathMsg(this->inputPathVis_, inputTrajPathMsg);
			visualization_msgs::msg::MarkerArray inputPathPoints;
	...
	geometry_msgs::msg::PoseStamped bsplineTraj::getPose(double t, bool yaw){
	...
	nav_msgs::msg::Path bsplineTraj::evalTrajToMsg(bool yaw){
		return this->evalTrajToMsg(this->ts_, yaw);
	}

	nav_msgs::msg::Path bsplineTraj::evalTrajToMsg(double dt, bool yaw){
	...
	void bsplineTraj::pathMsgToEigenPoints(const nav_msgs::msg::Path& path, std::vector<Eigen::Vector3d>& points){
		Eigen::Vector3d p;
	...
	void bsplineTraj::eigenPointsToPathMsg(const std::vector<Eigen::Vector3d>& points, nav_msgs::msg::Path& path){
