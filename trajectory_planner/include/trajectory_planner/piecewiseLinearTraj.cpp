...
namespace trajPlanner{
	pwlTraj::pwlTraj(const rclcpp::Node::SharedPtr& node) : node_(node){
		this->clock_ = node_->get_clock();
		this->logger_ = node_->get_logger();
	}

	void pwlTraj::updatePath(const nav_msgs::msg::Path& path, bool useYaw){
	...
	void pwlTraj::updatePath(const nav_msgs::msg::Path& path, double desiredVel, bool useYaw){
		std::vector<trajPlanner::pose> trajPath;
		for (const geometry_msgs::msg::PoseStamped &p: path.poses){
			trajPlanner::pose trajP;
			if (not useYaw){
	...
	void pwlTraj::makePlan(nav_msgs::msg::Path& trajectory, double delT){
		std::vector<geometry_msgs::msg::PoseStamped> trajVec;
		for (double t=0; t<this->desiredTime_.back(); t+=delT){
			geometry_msgs::msg::PoseStamped ps = this->getPose(t);
	...
	void pwlTraj::makePlan(std::vector<trajPlanner::pose>& trajectory, double delT){
		...
		// for last pose
		geometry_msgs::msg::PoseStamped ps = this->getPose(this->desiredTime_.back());
		double yaw = trajPlanner::rpy_from_quaternion(ps.pose.orientation);
		trajPlanner::pose p;
		p.x = ps.pose.position.x;
		p.y = ps.pose.position.y;
	...
	geometry_msgs::msg::PoseStamped pwlTraj::getPose(double t){
		...
		for (size_t i=0; i<this->desiredTime_.size()-1; ++i){
			...
			if ((t >= startTime) and (t <= endTime)){
				if (i % 2 == 1){ // it is in a rotation period
					...
					geometry_msgs::msg::Quaternion quat = trajPlanner::quaternion_from_rpy(0, 0, currYaw);
	...
	geometry_msgs::msg::PoseStamped pwlTraj::getFirstPose(){
		double yaw = this->path_[0].yaw;
		geometry_msgs::msg::Quaternion quat = trajPlanner::quaternion_from_rpy(0, 0, yaw);
