...
bool AStar::AstarSearch(const double step_size, Vector3d start_pt, Vector3d end_pt){
    rclcpp::Time time_1 = steady_clock_.now();
    ++rounds_;
    ...
    {
        RCLCPP_WARN(logger_, "[Path Search]: Unable to handle the initial or end point, force return!");
        return false;
    }
    ...
    while (!openSet_.empty())
    {
        ...
        rclcpp::Time time_2 = steady_clock_.now();
        if ((time_2 - time_1).seconds() > 0.2)
        {
            RCLCPP_WARN(logger_, "[Path Search]: Failed in A star path searching !!! 0.2 seconds time limit exceeded.");
            return false;
        }
    }
    ...
    if ((time_2 - time_1).seconds() > 0.1)
        RCLCPP_WARN(logger_, "[Path Search]: Time consume in A star path finding is %.3fs, iter=%d", (time_2 - time_1).seconds(), num_iter);
