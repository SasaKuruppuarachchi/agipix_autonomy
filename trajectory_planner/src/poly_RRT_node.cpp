...
class PolyRRTNode : public rclcpp::Node {
public:
	PolyRRTNode() : rclcpp::Node("Poly_RRT_test_node") {
		...
		clickedPointSub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
			...
			1000,
			std::bind(&PolyRRTNode::clickedPointCB, this, std::placeholders::_1),
			subOptions);
		...
		startVisTimer_ = this->create_wall_timer(
			...
			std::bind(&PolyRRTNode::publishStartVis, this),
			startVisCbGroup_);
		goalVisTimer_ = this->create_wall_timer(
			std::chrono::duration<double>(0.1),
			std::bind(&PolyRRTNode::publishGoalVis, this),
			goalVisCbGroup_);

		mainTimer_ = this->create_wall_timer(
			std::chrono::duration<double>(0.1),
			std::bind(&PolyRRTNode::mainTimerCB, this),
			mainCbGroup_);
...
int main(int argc, char** argv){
	...
	auto node = std::make_shared<PolyRRTNode>();
