...
class PolyRRTStarNode : public rclcpp::Node {
public:
	PolyRRTStarNode() : rclcpp::Node("Poly_RRT_star_test_node") {
		...
		clickedPointSub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
			...
			1000,
			std::bind(&PolyRRTStarNode::clickedPointCB, this, std::placeholders::_1),
			subOptions);
		...
		startVisTimer_ = this->create_wall_timer(
			...
			std::bind(&PolyRRTStarNode::publishStartVis, this),
			startVisCbGroup_);
		goalVisTimer_ = this->create_wall_timer(
			std::chrono::duration<double>(0.1),
			std::bind(&PolyRRTStarNode::publishGoalVis, this),
			goalVisCbGroup_);

		mainTimer_ = this->create_wall_timer(
			std::chrono::duration<double>(0.1),
			std::bind(&PolyRRTStarNode::mainTimerCB, this),
			mainCbGroup_);
...
int main(int argc, char** argv){
	...
	auto node = std::make_shared<PolyRRTStarNode>();
