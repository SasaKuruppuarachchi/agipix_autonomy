...
class PolyRRTGoalNode : public rclcpp::Node {
public:
	PolyRRTGoalNode() : rclcpp::Node("Poly_RRT_goal_test_node") {
		...
		clickedPointSub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
			...
			1000,
			std::bind(&PolyRRTGoalNode::clickedPointCB, this, std::placeholders::_1),
			subOptions);
		...
		startVisTimer_ = this->create_wall_timer(
			...
			std::bind(&PolyRRTGoalNode::publishStartVis, this),
			startVisCbGroup_);
		goalVisTimer_ = this->create_wall_timer(
			std::chrono::duration<double>(0.1),
			std::bind(&PolyRRTGoalNode::publishGoalVis, this),
			goalVisCbGroup_);

		mainTimer_ = this->create_wall_timer(
			std::chrono::duration<double>(0.1),
			std::bind(&PolyRRTGoalNode::mainTimerCB, this),
			mainCbGroup_);
		execTimer_ = this->create_wall_timer(
			std::chrono::duration<double>(1.0 / 50.0),
			std::bind(&PolyRRTGoalNode::execTimerCB, this),
			mainCbGroup_);
...
int main(int argc, char** argv){
	...
	auto node = std::make_shared<PolyRRTGoalNode>();
