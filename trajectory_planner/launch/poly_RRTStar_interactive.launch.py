    ...
    return LaunchDescription(
        [
            DeclareLaunchArgument(
                "map_location",
                default_value=PathJoinSubstitution([pkg_share, "map", "maze.bt"]),
            ),
            SetParameter(name="use_sim_time", value=False),
            Node(
                package="trajectory_planner",
                executable="poly_RRTStar_node",
                name="Poly_RRT_star_test_node",
                output="screen",
                parameters=params,
            ),
            Node(
                ...
                output="log",
                parameters=[{'octomap_path': octomap_path}, {'frame_id': 'map'}]  # <--- Pass as param her
            ),
            Node(
                package="rviz2",
                executable="rviz2",
